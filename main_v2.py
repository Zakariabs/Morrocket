...
pygame.display.set_caption("Ruimte schiet spel")
...
SECOND_YELLOW_SPACE_SHIP = pygame.image.load(os.path.join("assets", "ufo.png"))
...
BG = pygame.transform.scale(pygame.image.load(os.path.join("assets", "Space_BG_2.png")), (WIDTH, HEIGHT))
...
class Laser:
    ...
    def off_screen(self, height): # we gaan kijken of de laser van het scherm is
        return not(self.y <= height and self.y >= 0) # als de laser niet meer op het scherm is dan returnen we True om de laser te verwijderen

    def collision(self, obj): # we gaan kijken of de laser een object raakt
        return collide(self, obj) # we gaan de collide functie aanroepen en kijken of de laser en het object elkaar raken
...
class Ship:
    ...
    def draw(self, window): # we gaan het schip tekenen
        window.blit(self.ship_img, (self.x, self.y))  # we gaan de afbeelding van het schip tekenen
    ...
    def move_lasers(self, vel, obj): # we gaan de lasers laten bewegen
        ...
        for laser in self.lasers: # we gaan de lasers laten bewegen en kijken of ze van het scherm zijn
            ...
            if laser.off_screen(HEIGHT): # als de laser van het scherm is dan verwijderen we de laser
                self.lasers.remove(laser)
            elif laser.collision(obj): # als de laser een object raakt dan verwijderen we de laser
                obj.health -= 10
                self.lasers.remove(laser)

    def cooldown(self): # we gaan de cooldown functie aanroepen om de cooldown van de laser te regelen
        if self.cool_down_counter >= self.COOLDOWN: # als de cooldown counter groter is dan de cooldown dan zetten we de cooldown counter op 0
...
class Player(Ship):
    def __init__(self, x, y, health=100):
        ...
        self.mask = pygame.mask.from_surface(self.ship_img) # we gaan de mask van het schip aanmaken zodat we de collision kunnen regelen met de laser en het schip
        self.max_health = health # we gaan de max health van het schip aanmaken zodat we de healthbar kunnen tekenen in het spel
    ...
    def move_lasers(self, vel, objs):  # we gaan de lasers laten bewegen en kijken of ze van het scherm zijn en of ze een object raken en verwijderen ze als ze van het scherm zijn of als ze een object raken
        ...
        for laser in self.lasers: # we gaan de lasers laten bewegen en kijken of ze van het scherm zijn en of ze een object raken en verwijderen ze als ze van het scherm zijn of als ze een object raken
            ...
            if laser.off_screen(HEIGHT): # als de laser van het scherm is dan verwijderen we de laser
                ...
                for obj in objs:
                    if laser.collision(obj):  # als de laser een object raakt dan verwijderen we het object
                        objs.remove(obj)
                        if laser in self.lasers: # als de laser een object raakt dan verwijderen we de laser
                            self.lasers.remove(laser)

    def draw(self, window): # we gaan het schip tekenen
        super().draw(window) # we gaan de draw functie van de ship class aanroepen om het schip te tekenen
    ...
    def healthbar(self, window): # we gaan de healthbar tekenen in het spel
        pygame.draw.rect(window, (255,0,0), (self.x, self.y + self.ship_img.get_height() - 20, self.ship_img.get_width(), 10)) # we gaan de gele healthbar tekenen in het spel
        pygame.draw.rect(window, (255,255,0), (self.x, self.y + self.ship_img.get_height() - 20, self.ship_img.get_width() * (self.health/self.max_health), 10)) # we gaan de groene healthbar tekenen in het spel als



class Second_player(Ship):
    ...
    def healthbar(self, window):
        ...
        pygame.draw.rect(window, (255,255,0), (self.x, self.y + self.ship_img.get_height() - 20, self.ship_img.get_width() * (self.health/self.max_health), 10))
...
class Enemy(Ship): # we gaan de enemy class aanmaken zodat we de enemy kunnen aanroepen in het spel en de functies van de ship class kunnen gebruiken
    ...
    def __init__(self, x, y, color, health=100):
        super().__init__(x, y, health)
        self.ship_img, self.laser_img = self.COLOR_MAP[color] # we gaan de kleur van de enemy aanmaken zodat we de enemy kunnen aanmaken in verschillende kleuren
        self.mask = pygame.mask.from_surface(self.ship_img) # we gaan de mask van het schip aanmaken zodat we de collision kunnen regelen met de laser en het schip
    ...
    def shoot(self):  # we gaan de laser laten schieten
        if self.cool_down_counter == 0: # als de cooldown counter 0 is dan zetten we de cooldown counter op 1
            laser = Laser(self.x-20, self.y, self.laser_img) # we gaan de laser beam centreren op het schip
            self.lasers.append(laser) # we gaan de laser toevoegen aan de lasers lijst zodat we de laser kunnen tekenen in het spel
            self.cool_down_counter = 1 # we gaan de cooldown counter op 1 zetten zodat de laser niet te snel kan schieten
...
def collide(obj1, obj2): # we gaan kijken of de laser en het schip elkaar raken
    offset_x = obj2.x - obj1.x # we gaan de offset van de x positie van de laser en het schip berekenen zodat we de collision kunnen regelen
    offset_y = obj2.y - obj1.y  # we gaan de offset van de y positie van de laser en het schip berekenen zodat we de collision kunnen regelen
    return obj1.mask.overlap(obj2.mask, (offset_x, offset_y)) != None # we gaan kijken of de laser en het schip elkaar raken en als ze elkaar raken dan returnen we True en als ze elkaar niet raken dan returnen we False
...
def main():      # we gaan de main functie aanmaken zodat we het spel kunnen aanroepen
    ...
    main_font = pygame.font.SysFont("Arial", 50) # we gaan de font aanmaken zodat we de tekst kunnen aanpassen in het spel
    lost_font = pygame.font.SysFont("Arial", 60) # we gaan de font aanmaken zodat we de tekst kunnen aanpassen in het spel

    enemies = []     # we gaan de enemies lijst aanmaken zodat we de enemies kunnen aanmaken in het spel
    wave_length = 5  # we gaan de wave length aanmaken zodat we de enemies kunnen aanmaken in het spel
    ...
    player = Player(300, 630)   # we gaan het schip aanmaken zodat we het schip kunnen aanroepen in het spel
    second_player = Player(300, 630) # we gaan het schip aanmaken zodat we het schip kunnen aanroepen in het spel

    clock = pygame.time.Clock() # we gaan de clock aanmaken zodat we de snelheid van het spel kunnen aanpassen
    ...
    def redraw_window():    # we gaan de redraw window functie aanmaken zodat we de tekst en de schepen kunnen tekenen in het spel
        ...
        WIN.blit(lives_label, (10, 10)) # we gaan de lives label tekenen in het spel
        WIN.blit(level_label, (WIDTH - level_label.get_width() - 10, 10)) # we gaan de level label tekenen in het spel

        for enemy in enemies:
            enemy.draw(WIN)  # we gaan de enemies tekenen in het spel

        player.draw(WIN) # we gaan de spelers tekenen in het spel
        second_player.draw(WIN) # we gaan de spelers tekenen in het spel

        if lost: # als de speler verloren heeft dan gaan we de tekst tekenen in het spel
            lost_label = lost_font.render("JE HEBT VERLOREN", 1, (255,255,255))
            WIN.blit(lost_label, (WIDTH/2 - lost_label.get_width()/2, 350))  # we gaan de lost label tekenen in het midden v/t spel

        pygame.display.update() # we gaan de display updaten zodat we de tekst en de schepen kunnen tekenen in het spel
    ...
    while run:
        ...
        if lives <= 0 or player.health <= 0: # als de lives of de health van het schip 0 is dan gaan we de lost variabele op True zetten
            lost = True
            lost_count += 1 # we gaan de lost count op 1 zetten zodat we de tekst kunnen aanpassen in het spel
        ...
        if len(enemies) == 0: # als de enemies 0 zijn dan gaan we de level op 1 zetten en de wave length op 5 zetten zodat we de enemies kunnen aanmaken in het spel
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH-100), random.randrange(-1500, -100), random.choice(["red", "blue", "green"])) # we gaan de enemies aanmaken in het spel
                enemies.append(enemy) # we gaan de enemies toevoegen aan de enemies lijst
        ...
        if keys[pygame.K_a] and second_player.x - player_vel > 0:
            second_player.x -= player_vel
        if keys[pygame.K_d] and second_player.x + player_vel + second_player.get_width() < WIDTH:
            second_player.x += player_vel
        if keys[pygame.K_w] and second_player.y - player_vel > 0:
            second_player.y -= player_vel
        if keys[pygame.K_s] and second_player.y + player_vel + second_player.get_height() + 15 < HEIGHT:
            second_player.y += player_vel
        ...
        for enemy in enemies[:]: # we gaan de enemies aanmaken in het spel
            enemy.move(enemy_vel) # we gaan de enemy velocity aanmaken zodat we de snelheid van de enemies kunnen aanpassen in het spel
            enemy.move_lasers(laser_vel, player) # we gaan de enemy lasers aanmaken zodat we de snelheid van de enemy lasers kunnen aanpassen in het spel

        for enemy in enemies[:]:
            enemy.move(enemy_vel)
            enemy.move_lasers(laser_vel, second_player)


            if random.randrange(0, 2*60) == 1: # we gaan de random.randrange gebruiken zo dat de enemies 50% kans hebben om te schieten
                enemy.shoot()
            ...
            elif enemy.y + enemy.get_height() > HEIGHT: # als de enemy onderaan het scherm is dan gaan we de lives met 1 verminderen en de enemy verwijderen uit de enemies lijst
                lives -= 1 # we gaan de lives met 1 verminderen
                enemies.remove(enemy)  # we gaan de enemy verwijderen uit de enemies lijst


            if collide(enemy, second_player):
        ...
        player.move_lasers(-laser_vel, enemies) # we gaan de player lasers aanmaken zodat we de lasers van de player kunnen aanpassen in het spel

def main_menu():
    title_font = pygame.font.SysFont("comicsans", 70) # we gaan de titel font aanmaken zodat we de titel kunnen aanpassen in het spel
    run = True
    while run: # we gaan de main menu loop aanmaken zodat we het spel kunnen aanroepen
        WIN.blit(BG, (0,0)) # we gaan de achtergrond tekenen in het spel
        title_label = title_font.render("Click om te starten", 1, (255,255,255)) # we gaan de titel label aanmaken zodat we de titel kunnen aanpassen in het spel
        WIN.blit(title_label, (WIDTH/2 - title_label.get_width()/2, 350))
        pygame.display.update() # we gaan de display updaten zodat we de tekst en de schepen kunnen tekenen in het spel
        for event in pygame.event.get():    # we gaan de events aanmaken zodat we het spel kunnen afsluiten
            if event.type == pygame.QUIT: # als we op het kruisje klikken dan gaan we de run variabele op False zetten zodat we het spel kunnen afsluiten
                run = False
            if event.type == pygame.MOUSEBUTTONDOWN: # als we op de muis klikken dan gaan we de main functie aanroepen zodat we het spel kunnen aanroepen
                main()
    pygame.quit()
